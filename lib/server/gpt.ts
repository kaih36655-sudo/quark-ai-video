...
const normalizeGrokPlanDurationSeconds = (value: number, allowFlexibleSingleDuration = false) => {
  const allowed = allowFlexibleSingleDuration ? [5, 10, 15, 20, 30, 40, 50, 60] : [10, 20, 30, 40, 50, 60];
  return allowed.includes(value) ? value : 10;
};

const getGrokPlanTotalSegments = (targetDurationSeconds: number) =>
  targetDurationSeconds <= 15 ? 1 : Math.max(1, Math.min(6, targetDurationSeconds / 10));

const getGrokPlanSegmentBounds = (index: number, totalSegments: number, targetDurationSeconds: number) => {
  if (totalSegments === 1) return { startSecond: 0, endSecond: targetDurationSeconds };
  return { startSecond: index * 10, endSecond: (index + 1) * 10 };
};
...
const buildGrokSegmentPrompt = (params: {
  ...
  singleCreateDurationMode?: boolean;
}) => {
  ...
  const segmentTaskLine =
    params.singleCreateDurationMode
      ? `生成 ${params.targetDurationSeconds} 秒视频，单段完整呈现主题的开场、关键动作和收束。`
      : `生成 ${params.targetDurationSeconds} 秒连续视频，本段只推进完整故事的当前部分。`;
...
当前段任务：${params.singleCreateDurationMode ? `完整覆盖 0-${params.targetDurationSeconds}s，不要写成 10 秒脚本。` : "只推进完整故事的当前部分，不能把本段写成完整独立短视频。"}
...
const buildGrokPromptsFromSegmentPlan = (params: {
  ...
  const prompts = params.segmentPlan.map((segment) =>
    buildGrokSegmentPrompt({ ...params, segment, mode: "stitch", singleCreateDurationMode: params.singleCreateDurationMode })
  );
  return {
    basePrompt: buildGrokSegmentPrompt({ ...params, segment: params.segmentPlan[0], mode: "base", singleCreateDurationMode: params.singleCreateDurationMode }),
    extensionPrompts: params.segmentPlan.slice(1).map((segment) =>
      buildGrokSegmentPrompt({ ...params, segment, mode: "extend", singleCreateDurationMode: params.singleCreateDurationMode })
    ),
...
const grokMediumVideoFallback = (params: {
  ...
  const units = getGrokPlanTotalSegments(params.targetDurationSeconds);
  const ratioLabel = params.ratio === "9:16" ? "9:16竖屏" : "16:9横屏";
  ...
  const segmentPlan = storyBeats.map((storyBeat, index) => {
    const bounds = getGrokPlanSegmentBounds(index, units, params.targetDurationSeconds);
    return {
      segmentIndex: index + 1,
      startSecond: bounds.startSecond,
      endSecond: bounds.endSecond,
      storyBeat,
      visualAction:
        units === 1
          ? `画面从第 0 秒立即运动，在 ${params.targetDurationSeconds} 秒内完成主体动作、关键信息和结果收束`
          : index === 0
  ...
  const prompts = buildGrokPromptsFromSegmentPlan({
    ...
    singleCreateDurationMode: params.singleCreateDurationMode,
  });
  return {
    title,
    userTheme: theme,
...
export async function generateGrokMediumVideoPlan(params: {
  ...
  agentDescription?: string;
  agentConstraints?: string;
  allowFlexibleSingleDuration?: boolean;
}): Promise<GrokMediumVideoPlan> {
  const allowFlexibleSingleDuration = params.allowFlexibleSingleDuration === true;
  const targetDurationSeconds = normalizeGrokPlanDurationSeconds(params.targetDurationSeconds, allowFlexibleSingleDuration);
  const units = getGrokPlanTotalSegments(targetDurationSeconds);
  const isSingleSegment = units === 1;
  const isFlexibleSingleSegment = allowFlexibleSingleDuration && isSingleSegment;
  const userTheme = params.theme?.trim() || "用户主题";
  const agentConstraints = normalizeGrokAgentConstraints(params.agentConstraints, params.agentDescription);
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return grokMediumVideoFallback({ ...params, targetDurationSeconds, singleCreateDurationMode: isFlexibleSingleSegment });
        ...
        messages: [
          {
            role: "system",
            content:
              isFlexibleSingleSegment
                ? "你是 Grok 单段视频脚本策划专家。必须仅返回 JSON。为目标秒数生成一条单段完整视频故事、口播和 segmentPlan。segmentPlan 是唯一权威，必须使用真实目标时长，不得改写成 10 秒。"
...
${isFlexibleSingleSegment ? `单段时长：${targetDurationSeconds}秒` : `10秒单位数：${units}`}
扩展次数：${isFlexibleSingleSegment ? 0 : units - 1}
画面比例：${ratioLabel}

要求：
1. 第一层先写完整总脚本：overallStory/completeScript/fullVoiceoverScript 必须是一条完整视频故事和完整口播，不是分段独立文案集合。
2. ${isFlexibleSingleSegment ? `第二层输出 1 个 ${targetDurationSeconds} 秒 segmentPlan：startSecond 必须是 0，endSecond 必须是 ${targetDurationSeconds}，basePrompt 必须明确“生成 ${targetDurationSeconds} 秒视频”，禁止写成 10 秒、0-10s 或第 1/1 段 10秒。` : `第二层再切为 ${units} 个 10 秒 segmentPlan。每段只负责完整脚本的一部分，不允许重复介绍同一个卖点，不允许重新开头，不允许每段都像独立短视频。`}
...
5. ${isFlexibleSingleSegment ? `第 1/1 段必须在 ${targetDurationSeconds} 秒内完成场景建立、关键动作和自然收束。` : "第 1 段只负责场景建立、起因、痛点或冲突，不要提前讲结果。"}
...
${planAttempt > 1 ? "注意：上一版 segmentPlan 被判定相邻片段重复或缺少连续性，这次必须让每段 storyBeat/visualAction/voiceoverPart 明显不同。" : ""}`,
      ...
      clearTimeout(timer);
      if (!response.ok) return grokMediumVideoFallback({ ...params, targetDurationSeconds, singleCreateDurationMode: isFlexibleSingleSegment });
      const data = await response.json();
      const content = data?.choices?.[0]?.message?.content;
      if (!content || typeof content !== "string") return grokMediumVideoFallback({ ...params, targetDurationSeconds, singleCreateDurationMode: isFlexibleSingleSegment });
      ...
      if (!parsed.title || !Array.isArray(parsed.outline) || parsed.outline.length !== units || !Array.isArray(parsed.segmentPlan) || parsed.segmentPlan.length !== units) {
        return grokMediumVideoFallback({ ...params, targetDurationSeconds, singleCreateDurationMode: isFlexibleSingleSegment });
      }
      const segmentPlan = parsed.segmentPlan.map((item, index) => {
        const bounds = getGrokPlanSegmentBounds(index, units, targetDurationSeconds);
        return {
          segmentIndex: index + 1,
          startSecond: bounds.startSecond,
          endSecond: bounds.endSecond,
      ...
      const prompts = buildGrokPromptsFromSegmentPlan({
        ...
        agentDescription: params.agentDescription,
        agentConstraints,
        singleCreateDurationMode: isFlexibleSingleSegment,
      });
      return {
        ...
        outline: parsed.outline.map((item, index) => {
          ...
          const generatedSummary = `${segment?.storyBeat || ""}；${segment?.visualAction || ""}；口播：${segment?.voiceoverPart || ""}`;
          return {
            segmentIndex: index + 1,
            start: segment?.startSecond ?? 0,
            end: segment?.endSecond ?? targetDurationSeconds,
            summary: isFlexibleSingleSegment ? generatedSummary : typeof item.summary === "string" && item.summary.trim() ? item.summary.trim() : generatedSummary,
          };
        }),
      };
    }
